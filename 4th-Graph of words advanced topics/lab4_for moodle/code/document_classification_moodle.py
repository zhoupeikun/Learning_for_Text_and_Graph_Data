...
# stopWords and words less than 3 characters in length has been removed
index_remove = [i for i in range(len(train.ix[:, 1])) if (train.ix[i, 1] != train.ix[i, 1]) or (
(train.ix[i, 1] == train.ix[i, 1]) and (len(train.ix[i, 1].split(" ")) < 4))]
...
index_remove = [i for i in range(len(test.ix[:, 1])) if (test.ix[i, 1] != test.ix[i, 1]) or (
(test.ix[i, 1] == test.ix[i, 1]) and (len(test.ix[i, 1].split(" ")) < 4))]
...
print "min, max and average number of terms per document:", min(n_terms_per_doc), max(n_terms_per_doc), sum(
    n_terms_per_doc) / len(n_terms_per_doc)
...
for terms in terms_by_doc:
    all_graphs.append(terms_to_graph(terms, windows))
...
for i in xrange(len(all_graphs)):
    ...
    for term in list(set(terms_in_doc)):
        ...
        # store TF-IDF value
        feature_row_tfidf[index] = ((1 + math.log1p(1 + math.log1p(tf))) / (
        1 - 0.2 + (0.2 * (float(doc_len) / avg_len)))) * idf_term
...
for i in xrange(len(all_graphs_test)):
    ...
    for term in list(set(terms_in_doc)):
        ...
        # store TF-IDF value
        feature_row_tfidf_test[index] = ((1 + math.log1p(1 + math.log1p(tf))) / (
        1 - 0.2 + (0.2 * (float(doc_len) / avg_len)))) * idf_term
